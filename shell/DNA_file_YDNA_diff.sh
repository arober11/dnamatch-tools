#!/bin/bash
# Rough YDNA diff between AncestryDNA, myHeritage, FTDNA, or 23ANDME raw files
#
...
then
  ...
  echo
  echo "Purpose: Diff the YDNA calls between two DNA files."
  echo
...
INFILE1="$1"
INFILE1_YDNA="`basename $INFILE1`.YDNA"
INFILE2="$2"
INFILE2_YDNA="`basename $INFILE2`.YDNA"
...
function YDNA_extract_and_convert_NOCALLS {
  ...
  egrep -v "#|\/|\*|RSID,|,0,0,--" "$1" | egrep "\t24\t|\tY\t|,24," | $SED -e "s/--/0/" -e "s/AA/A/" -e "s/CC/C/" -e "s/TT/T/" -e "s/GG/G/" > "$2" 
}

YDNA_extract_and_convert_NOCALLS $INFILE1 $INFILE1_YDNA
YDNA_extract_and_convert_NOCALLS $INFILE2 $INFILE2_YDNA

if [ ! -s "$INFILE1_YDNA" ]
then
  echo "Error - $INFILE1 contains no YDNA calls"
  rm -f $INFILE1_YDNA $INFILE2_YDNA
  exit 3
fi

if [ ! -s "$INFILE2_YDNA" ]
then
  echo "Error - $INFILE2 contains no YDNA calls"
  rm -f $INFILE1_YDNA $INFILE2_YDNA
  exit 3
fi

diff $INFILE1_YDNA $INFILE2_YDNA > /dev/null
if [ $? -eq 0 ]
then
  echo "WARNING: files contain identical YDNA calls and formatting !!!"
  echo
fi
...
function diffCALLS {
  ...
  do
    ...
    # 'head -1 ' needed as a 23AndMe V5 file has 164 duplicate YDNA entries, under different rsid labels.
    if [ -z "$CALL2" ]
    then
        ((MISS+=1))
        if [ $MISSING -eq 1 ]
...
DELIM1="$DELIM"
countCalls "$INFILE1_YDNA"
guess_deliminator $INFILE2
DELIM2="$DELIM"
countCalls "$INFILE2_YDNA"
diffCALLS  "$INFILE1_YDNA" "$INFILE2_YDNA" "$DELIM1" "$DELIM2"
diffCALLS  "$INFILE2_YDNA" "$INFILE1_YDNA" "$DELIM2" "$DELIM1"
